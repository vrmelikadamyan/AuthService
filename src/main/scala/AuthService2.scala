...
object AuthService2 {
  ...
  // сервисы
  def notificationService = Notification2
  ...
  def startSignUp1(signUpDTO: Json) = {
    (for {
      data <- unpackJson(signUpDTO)
      email <- getEmail(data)
      login <- getField(data, "login")
      firstName <- getField(data, "firstName")
      secondName <- getField(data, "secondName")
      middleName <- getField(data, "middleName")
      password <- getField(data, "password")
    } yield {
      val token = createToken
      users += token -> User(login, firstName, secondName,
        middleName, email, Deactivated)
      createRegistrationRecord(login, password)
      sendToken(email, token)
      Map("success" -> s"user $login created").asJson
    }) match {
  ...
  def unpackJson(json: Json) =
    json.as[Map[String, String]].map(Right(_)).
      getOrElse(Left(Map("error" -> s"invalid JSON").asJson))

  def getEmail(data: Map[String, String]) =
    for {
      email <- getField(data, "email")
      validEmail <- validateEmail(email)
    } yield validEmail

  def getField(data: Map[String, String], field: String) =
    for {
      found <- findField(data, field)
      nonEmptyField <- isFieldEmpty(found)
    } yield nonEmptyField



  def findField(data: Map[String, String], field: String) =
    if (data.contains(field)) Right(data(field))
    else Left(Map("error" -> s"field $field not found").asJson)

  def isFieldEmpty(field: String) =
    if (field == "") Left(Map("error" -> s"field $field is empty").asJson)
    else Right(field)

  def validateEmail(email: String) = {
    if (email.matches("(\\w+[.-]?\\w+@\\w+\\.\\w+)")) Right(email)
    else Left(Map("error" -> "invalid email").asJson)
  }
    ...
    if (validateToken(token)) {
      if (users.contains(token)) {
        if (users(token).state == Deactivated) {
          ...
          Map("success" -> "user activated").asJson
